...
class NoiseGraphIfNode {
 ...
 private:
  ...
  void AddPostIfNode() {
    ...
    set_cond_params_ = {};
    set_cond_params_.func =
        reinterpret_cast<void*>(SetConditionKernel);
    set_cond_params_.gridDim = dim3(1);
    set_cond_params_.blockDim = dim3(1);
    set_cond_params_.sharedMemBytes = 0;
    set_cond_params_.kernelParams = set_cond_args_;
    set_cond_params_.extra = nullptr;

    cudaGraphNode_t cond_deps[] = {apply_node_};
    ...
    if_params.conditional.handle = post_handle_;
    if_params.conditional.type = cudaGraphCondTypeIf;
    if_params.conditional.size = 1;

    cudaGraphNode_t if_deps[] = {set_cond_node_};
    ...
    cudaGraph_t body_graph = if_params.conditional.phGraph_out[0];

    cudaKernelNodeParams post_params{};
    ...
    if (op_ == OperatorType::Linear) {
      ...
      post_params.gridDim = dim3(
          static_cast<unsigned int>((Cout_ + threads_ - 1) / threads_),
          static_cast<unsigned int>(linear_outer_));

      post_params.blockDim = dim3(threads_);
    ...
    post_params.sharedMemBytes = 0;
    post_params.kernelParams = post_args_;
    post_params.extra = nullptr;

    cudaGraphNode_t post_node = nullptr;
 ...
 private:
  ...
  cudaKernelNodeParams set_cond_params_{};
...
void run_case(const char* name, OperatorType op, unsigned int enable_post) {
  ...
  CUDA_CHECK(cudaMalloc(
      &d_workspace,
      NoiseGraphIfNode<T>::WorkspaceBytes(C)));
...
int main() {
  ...
  printf("Conditional Graph IF-Node needs CUDA 12.4+\n\n");

  run_case<float>("linear graph if-node", OperatorType::Linear, 0);
  run_case<float>("linear graph if-node", OperatorType::Linear, 1);

  run_case<float>("conv graph if-node", OperatorType::Conv, 0);
  run_case<float>("conv graph if-node", OperatorType::Conv, 1);
